...
# possible.
function(corrosion_internal_set_imported_location_legacy target_name base_property filename)
...
function(corrosion_internal_set_imported_location_deferred target_name base_property output_directory_property filename)
...
# Helper function to call corrosion_internal_set_imported_location_deferred while eagerly
# evaluating arguments.
# Refer to https://cmake.org/cmake/help/latest/command/cmake_language.html#deferred-call-examples
function(corrosion_internal_call_set_imported_location_deferred target_name base_property output_directory_property filename)
    message(STATUS "corrosion_internal_call_set_imported_location_deferred in ${CMAKE_CURRENT_SOURCE_DIR}")
    cmake_language(EVAL CODE "
        cmake_language(DEFER
            CALL
            corrosion_internal_set_imported_location_deferred
            [[${target_name}]]
            [[${base_property}]]
...
function(corrosion_internal_set_imported_location target_name base_property output_directory_property filename)
    if(CORROSION_RESPECT_OUTPUT_DIRECTORY)
        corrosion_internal_call_set_imported_location_deferred("${target_name}" "${base_property}" "${output_directory_property}" "${filename}")
    else()
        corrosion_internal_set_imported_location_legacy("${target_name}" "${base_property}" "${filename}")
    endif()
endfunction()

function(corrosion_internal_copy_byproduct_legacy target_name cargo_build_dir file_names)
    if(ARGN)
...
function(corrosion_internal_copy_byproduct_deferred target_name output_dir_prop_name cargo_build_dir file_names)
    if(ARGN)
        message(FATAL_ERROR "Unexpected additional arguments")
    endif()
...
function(corrosion_internal_call_copy_byproduct_deferred target_name output_dir_prop_name cargo_build_dir file_names)
    cmake_language(EVAL CODE "
        cmake_language(DEFER
            CALL
            corrosion_internal_copy_byproduct_deferred
            [[${target_name}]]
...
function(corrosion_internal_copy_byproducts target_name output_dir_prop_name cargo_build_dir filenames)
    if(CORROSION_RESPECT_OUTPUT_DIRECTORY)
        corrosion_internal_call_copy_byproduct_deferred("${target_name}" "${output_dir_prop_name}" "${cargo_build_dir}" "${filenames}")
    else()
        corrosion_internal_copy_byproduct_legacy("${target_name}" "${cargo_build_dir}" "${filenames}")
    endif()
endfunction()
